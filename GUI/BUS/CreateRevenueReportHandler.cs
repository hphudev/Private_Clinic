...
namespace BUS
{
    public class CreateRevenueReportHandler : CreateReportHandler
    {
            ...
            DataGridView dgvReport)
        {
            if (await IsExistInvoice(month, year))
            {
                DateTime[] createdReportDates = await GetCreatedDates(month, year,
                    "HOADON HD, CTKB CT", GetCreatedInvoiceDateConditionForSQL(month, year), 7);
        ...
        private async Task<bool> IsExistInvoice(int month, int year)
        {
            return (await CountRecord("HOADON HD, CTKB CT", GetCreatedInvoiceDateConditionForSQL(month,
                year))) != 0;
        }

        private string GetCreatedInvoiceDateConditionForSQL(int month, int year)
        {
