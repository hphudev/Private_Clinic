CREATE DATABASE QUANLYPHONGMACHTU1
GO
USE QUANLYPHONGMACHTU1
GO
...
(
	...
	NGAYKHAMBENH SMALLDATETIME CHECK (NGAYKHAMBENH >= Convert([date],GETDATE()) AND NGAYKHAMBENH IS NOT NULL), 
...
(
	...
	CONSTRAINT PRIMARYKEY UNIQUE (TENTHUOC, MADONVITINH), 
...
AS
	BEGIN
		...
		SET DOANHTHU = (
						   ...
						   WHERE HD.MACTKB = CT.MACTKB AND CONVERT([date],CT.NGAYKHAMBENH) = CONVERT([date],CTBCDOANHTHU.NGAYLAP)
					   )
	END
GO
...
insert into thamso values(N'Tiền khám', 1)
go
insert into thamso values(N'Số bệnh nhân tối đa', 1)
go
